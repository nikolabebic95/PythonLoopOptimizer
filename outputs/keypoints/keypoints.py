        ...
        for file_name in os.listdir(os.path.join(folder, sub_folder)):
            with open(os.path.join(folder, sub_folder, file_name), "r") as file:
                ...
                for line in lines:
                    ...
                    out_x = (math.fabs(((yy) - (y0)) * (x1) - ((xy) - (x0)) * (y1) + (xy) * (y0) - (yy) * (x0))) / (math.sqrt(((yy) - (y0)) ** 2 + ((xy) - (x0)) ** 2))
                    out_y = (math.fabs(((yx) - (y0)) * (x1) - ((xx) - (x0)) * (y1) + (xx) * (y0) - (yx) * (x0))) / (math.sqrt(((yx) - (y0)) ** 2 + ((xx) - (x0)) ** 2))
                    ...
                    print((os.path.join(sub_folder, file_name)), (i), out_x, out_y)
